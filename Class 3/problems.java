...
public class problems {
    ...
    public int permute(int[] nums, int ElementUsed, List<List<Integer>> ans, List<Integer> smallAns) {
        if (ElementUsed == nums.length) {
            List<Integer> base = new ArrayList<>(smallAns); // Deep Copy, else ans will be like this [[], []. []]
            ans.add(base);
            return 1;
        }
    ...
    private int combinationSum(int[] arr, int tar, int idx, List<List<Integer>> ans, List<Integer> smallAns) {
        ...
        for (int i = idx; i < arr.length; i++) {
            ...
            // Note : if array is sorted, then break here to make code a bit optimal
        }
        return count;
    }
    ...
    private int combinationSum2(int[] arr, int tar, int idx, List<List<Integer>> ans, List<Integer> smallAns) {
        ...
        int count = 0, prev = -1;
        for (int i = idx; i < arr.length; i++) {
            if (prev != arr[i] && tar - arr[i] >= 0) { // NOTE - on same level, if 2 values are same, may cause duplicate values, prev helps to override that
                smallAns.add(arr[i]);
